...
# replace the older `rofi -dmenumount` which had extra steps and couldn't handle
# encrypted drives.
...
chosen="$(echo "$alldrives" | rofi -dmenu -config ~/.config/rofi/rofidmenu.rasi -p "Mount which drive?" -i)"

# Function for prompting user for a mountpoint.
getmount(){
	mp="$(find /mnt /media /mount /home -maxdepth 1 -type d 2>/dev/null | rofi -dmenu -config ~/.config/rofi/rofidmenu.rasi -i -p "Mount this drive where?")"
	test -n "$mp"
	if [ ! -d "$mp" ]; then
		mkdiryn=$(printf "No\\nYes" | rofi -dmenu -config ~/.config/rofi/rofidmenu.rasi -i -p "$mp does not exist. Create it?")
...
case "$chosen" in
	...
	📱*)
		getmount
		echo "OK" | rofi -dmenu -config ~/.config/rofi/rofidmenu.rasi -i -p "Tap Allow on your phone if it asks for permission and then press enter"
		chosen="${chosen%%:*}"
		chosen="${chosen:1}"	# This is a bashism.
